...
ログインシェルで、リポジトリに新しいコミットが来ていれば知らせる。**pull は自分でやる。**

```
  New version is available!
  https://github.com/Khronos31/dotfiles
  git -C /home/user/dotfiles pull && /home/user/dotfiles/install.sh
```

**ネットワークはログインの経路に入れない。** ログイン時にやるのはキャッシュ1個の読み出しと
コミットの有無の確認だけで **約3ms**。実際の問い合わせはキャッシュが既定6時間より古いときだけ
バックグラウンドへ投げ、結果は次回のログインで効く。圏外でも回線が遅くてもログインは遅くならない
（毎回問い合わせる実装だと実測 0.5〜1.1秒、初回は5秒を超えることがあった）。

`fetch` ではなく `ls-remote` を使う。**リポジトリを一切書き換えない**ため。
「手元がリモートの先頭コミットを持っているか」で判定するので、手元が進んでいるときは黙る。

| 変数 | 既定 | 意味 |
|---|---|---|
| `DOTFILES_UPDATE_CHECK` | `1` | `0` で無効 |
| `DOTFILES_UPDATE_TTL` | `21600` | 問い合わせ間隔（秒） |
| `DOTFILES_DIR` | `$HOME/dotfiles` | リポジトリの場所 |

`~/.common_env.local` で上書きする。キャッシュは `${XDG_CACHE_HOME:-$HOME/.cache}/dotfiles-remote-head`。
